...
// Language Generator - Start
const langGen = new Blockly.Generator('LangGen');

workspace.addChangeListener(Blockly.Events.disableOrphans);

langGen['Thing/thing1'] = function(block) {
	function getBlockValues(root_block) {
		const returnBlock = root_block.getDescendants(true);
		if (returnBlock.length > 0) {
			let returnString = '';
			if (typeof returnBlock[0] !== 'undefined') {
				returnString = root_block.getFieldValue('lex') + ' {' + '\n';
				
				let child = root_block.getChildren(true)[0];
				while(child != null) {
					returnString += langGen.INDENT + langGen.blockToCode(child) + '\n';
	...
	return getBlockValues(block);
}

function getThing_parametersValue(wrapper_block){
	const returnBlock = wrapper_block.getDescendants(true);
	if (returnBlock.length > 0 && typeof returnBlock[0] !== 'undefined') {
		let child = wrapper_block.getChildren(true)[0];
		return langGen.blockToCode(child);
	} else {
		return 'Error';
	}
}
...
langGen['Thing_parameters/func'] = function (block) {
	return getThing_parametersValue(block);
}
...
langGen['When_sub_formula/When_sub_formula2'] = function (block) {
	return 'When_sub_formula/When_sub_formula2';
}
langGen['When_sub_formula/When_sub_formula1'] = function (block) {
	return 'When_sub_formula/When_sub_formula1';
}


langGen['Func/func4'] = function (block) {
	return 'Func/func4';
}
...
langGen['Call_sub_formula/star'] = function (block) {
	return 'Call_sub_formula/star';
}
langGen['Call_sub_formula/expression'] = function (block) {
	return 'Call_sub_formula/expression';
}
...
langGen['Params/params4'] = function (block) {
	return 'Params/params4';
}
langGen['Params/params3'] = function (block) {
	return 'Params/params3';
}
